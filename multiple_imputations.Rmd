...
df.miss.vars <- df.Conf.miss %>% select(c("monp", "mzp"))
...
mod.MImput.Conf <- with(MImput.df.Conf, lm(ssagp ~ mhp + gender + age_yrs_screen + race_ethn3 + income_family_annual))
...
mod.NoMiss <- summary(lm(ssagp ~ mhp + gender + age_yrs_screen + race_ethn3 + income_family_annual,  data = df.Conf.miss))
...
old_nocovari= lm(ssagp ~ cnp + cop + ecp + ecpt + hibp + mbp + mc1 + mcoh + mep + mhbp + mhh + mhht + mhnc + mhp + mib + mnp + moh + monp + mzp, data = exp_outcome)
