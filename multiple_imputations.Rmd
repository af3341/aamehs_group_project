...
all_var <- inner_join(exp_outcome, all_data, by = "seqn") |> 
  mutate(gender_female = if_else(gender == 1, 1, 0)) |>  #making a binary assignment of gender for multiple regression male = 0, female = 1
  mutate(eth_mex = case_when(race_ethn3 == 1 ~ 1, TRUE ~ 0),
         eth_hisp_oth = case_when(race_ethn3 == 2 ~ 1, TRUE ~ 0),
         eth_white = case_when(race_ethn3 == 3 ~ 1, TRUE ~ 0),
...
df.miss.vars <- df.Conf.miss %>% select(income_family_annual)
...
# What is the most common missing pattern? 
##*****************************************************************##
...
MImput.df.Conf <- mice(df.Conf.miss %>% select(-c("years_us", "education")),     # dataset
              m = 5,        # number of imputed datasets to create
              maxit = 50,   # number of iterations to create prediction models 
              meth = 'pmm', # method for imputation- depends on the class of missing variables
                            # pmm usually performs best for continuous variables
...
MImput.df.Conf$imp$income_family_annual
...
# all_exp_outcome has our original unimputed data
...
mod.MImput.Conf <- with(MImput.df.Conf, lm(ssagp ~ mhp + gender + age_yrs_screen + race_ethn1 + race_ethn3 + income_family_annual))
...
##** Class Question **##
# What are the 2 sources of variance when we use multiple imputation? 
##*****************************************************************##
...
# Let us look at just PM2.5 
pool(mod.MImput.Conf)[2,]

##** Class Question **##
# For the estimate of Beta for PM2.5, 
# which is greater: 
# variance within-imputation variance 
# or between-imputation variance? 
##*****************************************************************##
...
mod.NoMiss <- summary(lm(AveBMI ~ AvePM + MedHInc + PerBlack + PerLatinx + PerAsianAm + 
     MedHVal + LTHS + FemaleUnemp + MaleUnemp + ClimateRegion, data = df.complete))
...
##** Class Question **##
# Are the estimates of the two models similar? 
