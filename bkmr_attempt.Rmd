...
exposures <- select(exp_outcome, cnp:mzp)
...
fitkm <- kmbayes(y = exp_outcome$ssagp,   # outcome variable
...
```{r}
# i didn't run confounders though so idk if this means anything ...? or if i can assess convergence at all?
...
# im not sure if r seems converged...
...
ExtractPIPs(fitkm, z.names = c("cnp", "cop", "ecp", "ecpt", "hibp", "mbp", "mc1", "mcoh", "mbp", "mhbp", "mhh", "mhht", "mhnc", "mhp", "mib", "mnp", "moh", "monp", "mzp"))

#they all basically have the same importance.. 
...
pred.resp.univar <- PredictorResponseUnivar(fit = fitkm, 
                                            ...
                                            z.names = c("cnp", "cop", "ecp", "ecpt", "hibp", "mbp", "mc1", "mcoh", "mep", "mhbp", "mhh", "mhht", "mhnc", "mhp", "mib", "mnp", "moh", "monp", "mzp")) 
...
#this takes a long time to run i dont remember how long ~1hr
responses.overall <- OverallRiskSummaries(fit = fitkm,     # model 
                                      y = dta$ssagp,   # outcome 
                                      Z = exposures,       # exposures
                                      X = NULL, # matrix of confounders 
...
ggplot(responses.overall, aes(quantile, est, ymin = est - 1.96*sd, 
                              ymax = est + 1.96*sd)) + 
  geom_pointrange() + 
  ylab("Change in Expected AGP") + 
  plotTheme
