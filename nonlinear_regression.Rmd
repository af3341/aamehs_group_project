...
demo_new = demo |> 
  select(seqn, gender, age_yrs_screen, race_ethn1, race_ethn3, income_family_annual)
...
demographic_model = lm(ssagp ~ gender_female + age_yrs_screen + income_family_annual + eth_mex + eth_hisp_oth + eth_black + eth_asian + eth_nonhispanic_multirace, data = all_exp_outcome)

summary(demographic_model)

#Adding covariates to simple multiple regression 
lin_covari_model = lm(ssagp ~ cnp + cop + ecp + ecpt + hibp + mbp + mc1 + mcoh + mep + mhbp + mhh + mhht + mhnc + mhp + mib + mnp + moh + monp + mzp + gender_female + age_yrs_screen + income_family_annual + eth_mex + eth_hisp_oth + eth_black + eth_asian + eth_nonhispanic_multirace, data = all_exp_outcome)
...
total_covari_model <- gam(ssagp  ~ s(cnp) + s(cop) + s(ecp) + s(ecpt) + s(hibp) + s(mbp) + s(mc1) + s(mcoh) + s(mep) + s(mhbp) + s(mhh) + s(mhht) + s(mhnc) + s(mhp) + s(mib) + s(mnp) + s(moh) + s(monp) + s(mzp) + gender_female + age_yrs_screen + income_family_annual + eth_mex + eth_hisp_oth + eth_black + eth_asian + eth_nonhispanic_multirace, data = all_exp_outcome)
...
#Fit natural splines for metabolites with edf not equal to 1 - ecpt, mcoh, mep, mhht, mhnc, mhp, mnp, mzp

# Fit lm model for ecpt with natural spline
ecpt_ns_2 <- lm(ssagp ~ ns(ecpt, df = 2), data = exp_outcome)
ecpt_ns_3 <- lm(ssagp ~ ns(ecpt, df = 3), data = exp_outcome)
ecpt_ns_4 <- lm(ssagp ~ ns(ecpt, df = 4), data = exp_outcome)
ecpt_ns_5 <- lm(ssagp ~ ns(ecpt, df = 5), data = exp_outcome)
...
# model with df 3 has the lowest AIC 

# construct predictions based on model 
predagp.ecpt.ns <- predict(ecpt_ns_3, se.fit = TRUE, type = "terms")
...
predagp.ecpt.ns  <- predagp.ecpt.ns |> 
  mutate( pred.centered = ns.ecpt..df...3.,
          se = ns.ecpt..df...3..1)
...
predagp.ecpt.ns <- predagp.ecpt.ns |> bind_cols(exp_outcome)
...
moh_ns_2 <- lm(ssagp ~ ns(moh, df = 2), data = exp_outcome)
moh_ns_3 <- lm(ssagp ~ ns(moh, df = 3), data = exp_outcome)
moh_ns_4 <- lm(ssagp ~ ns(moh, df = 4), data = exp_outcome)
moh_ns_5 <- lm(ssagp ~ ns(moh, df = 5), data = exp_outcome)
...
# construct predictions based on model 
predagp.moh.ns <- predict(moh_ns_3, se.fit = TRUE, type = "terms")
...
predagp.moh.ns  <- predagp.moh.ns |> 
  mutate( pred.centered = ns.moh..df...3.,
          se = ns.moh..df...3..1)
...
predagp.moh.ns <- predagp.moh.ns |> bind_cols(exp_outcome)
