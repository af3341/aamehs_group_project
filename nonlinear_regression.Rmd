...
#edf not equal to 1 for ecpt, mcoh, mep, mhht, mhnc, mhp, mnp
```


```{r}
#fit ecpt model with penalized spline 
...
#construct predictions based on the model 
pred.ecpt <- predict(ecpt_model, se.fit = TRUE, type = "terms")

#convert to dataframe 
pred.ecpt <- as.data.frame(pred.ecpt)

#combine predictions and standard errors 

pred.ecpt <- pred.ecpt |> 
  mutate(pred.centered = s.ecpt.,
          se = s.ecpt..1)
...
pred.ecpt <- cbind(exp_outcome, pred.ecpt)
#having problem binding datasets because of different number of rows

#next steps: uncenter data, plot 
```


```{r}
#fit mcoh model with penalized spline 
...
#construct predictions based on the model 
pred.mcoh <- predict(mcoh_model, se.fit = TRUE, type = "terms")

#convert to dataframe 
pred.mcoh <- as.data.frame(pred.mcoh)
...
pred.mcoh <- cbind(exp_outcome, pred.mcoh)
#having problem binding datasets because of different number of rows
