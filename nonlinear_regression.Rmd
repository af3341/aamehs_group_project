...
#Old version 
lin_covari_model2 = lm(ssagp ~ cnp + cop + ecp + ecpt + hibp + mbp + mc1 + mep + mhbp + mhh + mhht + mib + moh + monp + mzp + gender_female + age_yrs_screen + income_family_annual + eth_mex + eth_hisp_oth + eth_black + eth_asian + eth_nonhispanic_multirace, data = all_exp_outcome)
...
```{r}
#Combine all plots
combined_plots_splines <- grid.arrange(ecpt_s, mep_s, mhht_s, mib_s, mhh_s, mzp_s, ncol = 3, top = "Non-Linear Models of Metabolite Exposure-Response Curves with Penalized Splines")
...
total_covari_model <- gam(ssagp  ~ s(cnp) + s(cop) + s(ecp) + s(ecpt) + s(hibp) + s(mbp) + s(mc1) + s(mep) + s(mhbp) + s(mhh) + s(mhht) + s(mib) + s(moh) + s(monp) + s(mzp) + gender_female + age_yrs_screen + income_family_annual + race_ethn3, data = all_exp_outcome)
...
#   geom_point() + 
#   geom_smooth(aes(y = predicted_ssagp), method = "lm", se = FALSE, color = "blue") +
#   labs(x = "MCOH (ug/L)", y = "AGP (mg/mL)", title = "MCOH") + 
#   theme_classic()
...
mhh_l <- ggplot(data_mhh, aes(x = mhh, y = ssagp)) +
  geom_point() + 
  geom_smooth(aes(y = predicted_ssagp), method = "lm", se = FALSE, color = "blue") +
  labs(x = "MHH (ug/L)", y = "AGP (mg/mL)", title = "MHH") + 
  theme_classic()
```
...
#   geom_point() + 
#   geom_smooth(aes(y = predicted_ssagp), method = "lm", se = FALSE, color = "blue") +
#   labs(x = "MHNC (ug/L)", y = "AGP (mg/mL)", title = "MHNC") + 
#   theme_classic()
```
...
#   geom_point() + 
#   geom_smooth(aes(y = predicted_ssagp), method = "lm", se = FALSE, color = "blue") +
#   labs(x = "MHP (ug/L)", y = "AGP (mg/mL)", title = "MHP") + 
#   theme_classic()
```
...
#   geom_point() + 
#   geom_smooth(aes(y = predicted_ssagp), method = "lm", se = FALSE, color = "blue") +
#   labs(x = "MNP (ug/L)", y = "AGP (mg/mL)", title = "MNP") + 
#   theme_classic()
```
...
linear_plots_combined <- grid.arrange(cnp_l, cop_l, ecp_l, ecpt_l, hibp_l, mbp_l, mc1_l, mep_l, mhbp_l, mhh_l, mhht_l, mib_l, moh_l, monp_l, mzp_l, ncol = 4, top = "Linear Models of Metabolite Exposure-Response Curves")
...
# create table 1 with age, gender, race/ethnicity, and family income

# recode age into age group 
all_exp_outcome <- all_exp_outcome |> 
...
# recode family income into fewer categories 
all_exp_outcome <- all_exp_outcome |> 
  mutate(income_family_combined = case_when(
                                      income_family_annual == 1 ~ "1",
                                      income_family_annual == 2 ~ "1",
                                      income_family_annual == 3 ~ "1",
                                      income_family_annual == 4 ~ "1",
                                      income_family_annual == 5 ~ "2",
                                      income_family_annual == 6 ~ "2",
                                      income_family_annual == 7 ~ "2",
                                      income_family_annual == 8 ~ "3",
                                      income_family_annual == 9 ~ "3",
                                      income_family_annual == 10 ~ "3",
                                      income_family_annual == 14 ~ "4",
                                      income_family_annual == 15 ~ "4",
                                      income_family_annual %in% c(77, 99) ~ "Missing",
                                      TRUE ~ NA_character_ ))

all_exp_outcome$income_family_combined <- ifelse(is.na(all_exp_outcome$income_family_combined), "Missing", all_exp_outcome$income_family_combined)
...
all_exp_outcome <- all_exp_outcome %>%
  mutate(
    age_yrs_screen = factor(age_yrs_screen),
    age_group = factor(age_group),
    gender = factor(gender),
    race_ethn3 = factor(race_ethn3),
    income_family_combined = factor(income_family_combined),
  )
...
all_exp_outcome <- all_exp_outcome |> 
  mutate(
    ...
    race_ethn3 = fct_recode(race_ethn3,
                         ...
                         "Other" = "6", 
                         "Missing" = "7"), 
    income_family_combined = fct_recode(income_family_combined,
                                  "$0 to $19,999" = "1", 
                                  "$20,000 to $44,999" = "2", 
                                  "$45,000 to $74,999" = "3", 
                                  "$75,000 and Over" = "4", 
...
variables_of_interest <- c("age_yrs_screen", "gender", "race_ethn3", "income_family_combined")


# Generate summary statistics for all variables
table_1 <- all_exp_outcome |> 
  ...
  tbl_summary(
    ...
    type = list(age_yrs_screen ~ "categorical",
                gender ~ "categorical",
                income_family_combined ~ "categorical",
                race_ethn3 ~ "categorical"),
    label = list(age_yrs_screen ~ "Age",
                 gender ~ "Gender",
                 income_family_combined ~ "Family Income",
...
write.xlsx(as.data.frame(table_1), file = "table_1.xlsx")


```
=======
The main metabolites for the first principal component are mhh, moh, ecp. The main metabolites for the second PC are monp, cop. The main metabolites for the third PC are mbp, mhbp, mib, and hibp.
>>>>>>> c2b72e073ebd3511b3a20ce2250ac93604ec0296
