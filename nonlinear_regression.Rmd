...
```{r}
#Combine all plots
gridExtra::grid.arrange(ecpt_s, mcoh_s, mep_s, mhht_s, mib_s, mhnc_s, mhh_s, mzp_s, ncol = 3, top = "Non-linear models of metabolite exposure-response curves with penalized splines")
```
...
all_exp_outcome <- all_exp_outcome[all_exp_outcome$cnp <= 1e-04, ]
```
...
mhpb_model <- lm(ssagp ~ mhbp + gender_female + age_yrs_screen + income_family_annual + race_ethn3, data = all_exp_outcome)
...
ggplot(data_cnp, aes(x = cnp, y = ssagp)) +
  geom_point() + 
  geom_smooth(aes(y = predicted_ssagp), method = "lm", se = FALSE, color = "blue") +
  labs(x = "CNP", y = "SSAGP", title = "CNP and SSAGP")
