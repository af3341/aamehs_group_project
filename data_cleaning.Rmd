...
# Let's see how to treat the NAs

metabolite_exploration <- phthte_urine %>% 
  select(cnp, cop, ecp, ecpt, hibp, mbp, mc1, mcoh, mep, mhbp, mhh, mhht, mhnc, mhp, mib, mnp, moh, monp, mzp) %>% 
  head(n = 100)

metabolite_exploration

# It seems that for subjects with NAs, we just don't have data on the entire subject, so we're just going to exclude them from the dataset.
...
#  percent_detect = ~sum(. != 0, na.rm = TRUE) / length(.) * 100
))
