...
dim(metabolite_matrix)

metabolite_filtered <- metabolite_matrix

percentiles_95 <- metabolite_filtered %>%
  summarise(across(everything(), ~quantile(., probs = 0.95, na.rm = TRUE), .names = "p95_{.col}"))

for(colname in names(metabolite_filtered)) {
